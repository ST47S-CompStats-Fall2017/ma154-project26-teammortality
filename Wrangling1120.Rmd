...
```{r}
require(dplyr)
load("~/Desktop/CompStats/Project/ma154-project26-teammortality/USA_county_data.RData")
...
```{r}
library(readxl)
ruralurbancodes2013 <- read_excel("ruralurbancodes2013.xls")
...
```{r}
mortalityrates <- mortalityrates %>% filter(fips>100) %>% arrange(fips)

require(dplyr)
countyrates <- full_join(newcountydata,mortalityrates,by=c("fips"))
```
